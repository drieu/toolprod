...
class ArenaParser extends Parser{
    ...
    def parse(File file) {
        ...
        InputStream arenaStream = new FileInputStream(file)
        BufferedReader br = new BufferedReader(new InputStreamReader(arenaStream))
        result = EMPTY
        try {
            ...
            if (app != null) {
                ...
                if (!arenaBean.fimPath.isEmpty()) {
                    ...
                    if (!errorImports.contains(arenaBean.appName)) {
                        log.info("save() possible error import from arena xml files :" + arenaBean.appName + " chemin :" + arenaBean.arenaPath)
